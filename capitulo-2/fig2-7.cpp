...
double d,dd,theta,h,t,Tr,fr,phi,VarreduraDoppler;
...
double f(double a1,double a2,double a12,double b12,int i)
{
    ...
    if (i==3) return -(d-dd)*b12-Omega*(a2-a1)*sin(alpha)           -0.5*q22*a12  -a12*gama;
    if (i==4) return  (d-dd)*a12+Omega*(a2-a1)*cos(alpha)           -0.5*q22*b12  -b12*gama;
    ...
    for (i=1;i<=2*Pulsos+1;i++)
       {
        ...
        for (m=1;m<=4;m++)
          c[m]=c[m]+b[m]*exp(-0.5*(dd)*(dd)/(2*Pi*LarguraDoppler*1e6)/
                                            (2*Pi*LarguraDoppler*1e6));                
      
        a[1]=a10;a[2]=0;a[3]=0;a[4]=0;
        ...
        fprintf(arquivo,"%12.10f %12.10f %12.10f %12.10f %12.10f %12.10f",
